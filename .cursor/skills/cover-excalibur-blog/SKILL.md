...
4. **Title zone sacred + whole collage (HARD)** — Derouter REST рисует **цельную** editorial collage: torn paper, заголовок, Wordstat, телефон, мемы в одном PNG. Host LARGE LEFT i2i. PIL overlay — только аварийный fallback, не основной путь.
...
# Цельная collage через Derouter (как B02/B04). PIL — только аварийный fallback.
python3 scripts/excalibur_blog_quad_regen_panels.py --article-dir "$ARTICLE" --slots cover
