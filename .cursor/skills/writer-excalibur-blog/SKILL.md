...
1. **Dzen engagement** — читай `shared/dzen-engagement-lock.md`: CTR (Title), read-through (лид 4–6 без рубки; HIT casus+число в первой строке; ~1400–1600 слов; **тройной пересказ = FAIL**), comments (bipolar-вопрос после финала), subs (голос Святослава, early TG+MAX, agency ending). **Casus specificity:** конкретный день в комнате; **ban** meta-disclaimer composite/anonymized («случай собирательный», «без фамилий», «механика повторяется»). **Plain language:** kitchen-table для обычных людей; **ban** академический/lawyer-blog ритм, стопки терминов, «заумно»; термин → сразу простым русским; **не** снимать heat casus чеклистом. **Klyshin:** только энергия первой строки; сюжеты свои. **Newbuild focus:** `shared/newbuild-focus-lock.md`. **Запрещено:** TL;DR, «Быстрый инсайт», bullets до первого H2, checklist/how-to в лиде; рубка 1–2 предложения; table+list+«главный вывод» в хвосте; self-score 9.0 loop; квота 1800+ слов; копирование кейсов Klyshin / TG-дампы.
...
- **No composite disclaimer (FAIL):** casus = конкретный день в комнате; **не** писать «случай собирательный», «без фамилий/адреса ЖК/названия банка», «механика в Тюмени повторяется», modeled/anonymized/«не репортаж». Не выдумывать реальные ЖК/банк; не объяснять читателю отсутствие имён.
- **Plain language (kitchen-table):** просто для обычных людей; **сохраняй** casus-тон и HIT первой строки. **Ban:** академический/lawyer-blog cadence, стопки юртерминов, «заумно». Термин — только если нужен, сразу простым русским. **Не** превращать в чеклист / lecture и **не** снимать heat news-casus.
