...
| DEROUTER_IMAGE_QUALITY=high | yes (local) | Legacy Derouter image fallback; grsai uses `GRSAI_IMAGE_QUALITY=high` |
...
| Image API key (grsai grsai standard image model) | required | `GRSAI_API_KEY` в Cloud Secrets |
| Image API key (Derouter text + optional image fallback) | required | `DEROUTER_API_KEY` в Cloud Secrets |
