...
**Не пиши H1 моделью Cursor.** Собери `--user-file` из research + Scout handoff; Derouter powerful tier (gpt-6-astra):
...
Заголовок — **новостной headline актуалочки** в ритме Klyshin (Дзен «любит новости» + вовлечение: лайки, комментарии, подписки),
но факты и город — **Святослав Шакин / Тюмень**. Не копипаст канала Клышина.

- **Цель H1:** открыть hot news-casus с конкретными stakes и намёком на финал — не чеклист, не TL;DR.
- **Comment magnet:** намечь в `title-brief.json` поле `comment_magnet_angle` — острый вопрос/угол спора для комментариев (Writer/Sol вшивают в текст).
