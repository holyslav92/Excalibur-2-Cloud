...
Dual CTA (мягко): «напишите на консультацию» **или** «сразу к делу / подключусь в сделку». Плюс **полный набор:** TG, MAX, **главная сайта** (`href="/"` или `{{SITE_BASE}}/`), Дзен, VK, гайды (`/gajdy/`), about (`/rieltor-tyumen/`). Gate `end_cta_full_channels` FAIL без ссылки на главную — не только gajdy/rieltor. Дзен + TG — как «смотреть разборы», не корпоративная выдумка. About/contact — только реальные URL из repo (`/rieltor-tyumen/`, `/kontakty/`).
